	...
	<div class="container">
		<?php global $opt_themes; if($opt_themes['sidebar_activated_']) { ?><div class="row"><?php } else { ?><?php } ?>
			...
			<nav class="overflow-auto d-flex mb-4"  >
				<a class="btn btn-light text-nowrap mr-2 <?php if( isset($_GET["orderby"]) && trim($_GET["orderby"]) == 'modified' && isset($_GET["order"]) && trim($_GET["order"]) == 'DESC' ){ echo 'active'; } ?>" href="?orderby=modified&order=DESC">Latest Updates</a>
				<a class="btn btn-light text-nowrap mr-2 <?php if( isset($_GET["orderby"]) && trim($_GET["orderby"]) == 'date' && isset($_GET["order"]) && trim($_GET["order"]) == 'ASC' ){ echo 'active'; } ?>" href="?orderby=date&order=ASC">New Releases</a> 
				<a class="btn btn-light text-nowrap mr-2 <?php if( isset($_GET["orderby"]) && trim($_GET["orderby"]) == 'views' && isset($_GET["order"]) && trim($_GET["order"]) == 'ASC' ){ echo 'active'; } ?>" href="?orderby=views&order=ASC">Popular</a>
			...
			<div class="mb-4 entry-content">
				<p>Discover thousands of unique mobile <?php printf( __( '%s', 'moddroid' ), '<b>' . single_cat_title( '', false ) . '</b>' ); ?>. We are constantly updating daily, along with the best mods available here</p>
			</div>					
